...
<form id="sss" class = "catebox" action="/bookjeok/recate" method="get">
...
style="color: #fff; background:#8181F7; 
font-size:1em; border-radius:0.5em; 
padding:5px 20px; margin-left:300px;"><a href="/bookjeok/arlist">오디오북</a></button>
...
<form id="idform" class="searchform" action="/bookjeok/ebooksearch" method="post" >
<% if(location > 0) { %>
...
<h4 align="center" style="font-size:3em">E-Book북 리뷰 목록</h4>
...
<% for(ReviewList r : list) { %>
	...
	<tr>
		<td rowspan="6" class="usimg"><a href="/bookjeok/rdetaile?booknm=<%= r.getBookNm() %>">
		<img id="ebookbookimg" src="/bookjeok/resources/book_img/<%= r.getBookNm() %>.png"></a></td>
		<td colspan="2" class="uslist"><a href="/bookjeok/rdetaile?booknm=<%= r.getBookNm() %>"><%= r.getBookNm() %></a></td>
	</tr>	
	<tr>
		<td colspan="2" class="uslist"><%= r.getAuthor() %> | <%= r.getPublisher() %> | <%= r.getPublishDate() %>일 출간</td>
	</tr>
	<tr>
		<td colspan="2" class="uslist">평점 : <%= r.getScore() %>점</td>
	</tr>
	<tr>
