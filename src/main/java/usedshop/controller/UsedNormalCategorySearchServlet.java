 ...
 */
@WebServlet("/usnmcatesearch")
public class UsedNormalCategorySearchServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public UsedNormalCategorySearchServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch (searchtype) {
		case "bookname":
			list = usservice.selectNormalCategorySearchBookName(keyword, category);
			break;
		case "writerid":
			list = usservice.selectNormalCategorySearchWriterId(keyword, category);
			break;
		}
		...
		if (list.size() > 0) {
			view = request.getRequestDispatcher("views/usedshop/normalListView.jsp");
