 ...
 */
@WebServlet("/usauhotlist")
public class UsedAuctionHotListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public UsedAuctionHotListServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 사용자 중고장터 경쟁 품목 (입찰 횟수 상위 Top20) 목록 출력 서블렛
		...
		list = usservice.selectHopBidList();

		// 4. 받은 결과로 성공/실패 페이지 내보내기
		RequestDispatcher view = null;
		if (list.size() > 0) {
			view = request.getRequestDispatcher("views/usedshop/auctionHotListView.jsp");
